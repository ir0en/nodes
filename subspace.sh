...
User=$USER
Type=simple
ExecStart=$(which subspace-node) --chain gemini-1 --execution wasm --keep-blocks 1024 --pruning 1024 --validator --name $SUBSPACE_NODENAME --base-path /mnt/nvme2/subspace
Restart=on-failure
LimitNOFILE=65535
...
User=$USER
Type=simple
ExecStart=$(which subspace-farmer) --base-path /mnt/nvme2/subspace-farmer farm --reward-address $SUBSPACE_WALLET --plot-size 400G
Restart=on-failure
LimitNOFILE=65535
